...
from gnucash import Session, GncNumeric, ACCT_TYPE_TRADING,\
    ACCT_TYPE_EXPENSE, ACCT_TYPE_INCOME, SessionOpenMode
...
class GnucashInterface():
    ...
    def get_root_commodity(self):
        return self.root.lookup_by_name("Assets").GetCommodity()
    ...
    def get_flow(self, account):
        ...
        return self.gnc_numeric_to_python_decimal(
            account.GetBalanceChangeForPeriod(date_start, date_end, True)
        )

    def get_balance(self, account):
        return self.gnc_numeric_to_python_decimal(
            account.GetBalanceAsOfDateInCurrency(
                self.assess_datetime,
                self.get_root_commodity(),
                True)
        )
...
#        all_accounts = chlds_inc + chlds_exp
    ...
    def get_expenses(self, depth_level):
        return self.get_account_level(
            self.root.lookup_by_name(self.expense_name),
            depth_level
        )

    def get_income(self, depth_level):
        return self.get_account_level(
            self.root.lookup_by_name(self.income_name),
            depth_level
        )
    ...
    def get_all_accounts(self, base_account, depth_level):
        base_acc = self.root.lookup_by_name(base_account)
        accounts = base_acc.get_descendants()
        output = []
        for acc in accounts:
    ...
    def set_assessment_datetime(self, date_time: str):
        if date_time == "current":
            ...
            self.assess_datetime = datetime(int(parts[2]),
                                            int(parts[1]),
                                            int(parts[0]))

    def get_period_limits(self):
        if self.assess_period == "month":
            start = datetime(self.assess_datetime.year,
                             self.assess_datetime.month, 1)
            end = datetime(self.assess_datetime.year,
                           self.assess_datetime.month+1, 1)
